...
LOCK_PATH = ROOT / "ARP22_POS3_BROAD_CONTEMPORARY_POSITIONING_LOCK_20260915.json"
OUT = ROOT / "results"
...
SESSION.headers.update({"User-Agent": "MarketsGPT-ARP22-POS3/1.0"})
...
def main():
    ...
    base=stat_block(net,dates,22301); s15=stat_block(n15,dates,22311); s2=stat_block(n2,dates,22312); gs=stat_block(gross,dates,22313)
    ...
    close={"schema":"mgpt_arp22_pos3_development_closeout_v1","generation":"ARP22-POS3","generatedAtUtc":datetime.now(timezone.utc).isoformat(),
           ...
           "nextAction":"Open frozen fresh validation only." if not failed else "Close ARP22-POS3; validation and holdout remain sealed."}
    trans={"schema":"mgpt_arp22_pos3_transport_diagnostics_v1","generation":"ARP22-POS3","generatedAtUtc":datetime.now(timezone.utc).isoformat(),
           "checksumRequired":True,"manifestCount":len(manifest),"symbols":diag}
    cp=OUT/"ARP22_POS3_DEVELOPMENT_CLOSEOUT_20260915.json"; tp=OUT/"ARP22_POS3_TRANSPORT_DIAGNOSTICS_20260915.json"
    cp.write_text(json.dumps(close,indent=2)+"\n"); tp.write_text(json.dumps(trans,indent=2)+"\n")
    sums=[f"{hashlib.sha256(p.read_bytes()).hexdigest()}  {p.name}" for p in sorted(OUT.glob("ARP22_POS3_*_20260915.json"))]
    (OUT/"ARP22_POS3_SHA256SUMS_20260915.txt").write_text("\n".join(sums)+"\n")
