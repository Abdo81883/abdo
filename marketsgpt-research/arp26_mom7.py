...
LOCK_PATH=ROOT/'ARP26_MOM7_WEEKLY_CROSS_SECTION_LOCK_20260915.json';OUT=ROOT/'results';OUT.mkdir(parents=True,exist_ok=True)
LOCK=json.loads(LOCK_PATH.read_text());SYMS=LOCK['universe']['symbols'];BASE='https://data.binance.vision/data/futures/um/monthly/klines'
WARM=datetime.fromisoformat(LOCK['windows']['warmupStart'].replace('Z','+00:00'));DEV_START=datetime.fromisoformat(LOCK['windows']['development']['start'].replace('Z','+00:00'));DEV_END=datetime.fromisoformat(LOCK['windows']['development']['endExclusive'].replace('Z','+00:00'));DAY=86400000
SESSION=requests.Session();SESSION.headers.update({'User-Agent':'MarketsGPT-ARP26-MOM7/1.0'})
...
def monday_grid(a,b):
 ...
 return out
def main():
 data,manifest,diag=transport();grid=monday_grid(DEV_START,DEV_END);prev=np.zeros(len(SYMS));gross=[];tos=[];dates=[];elig_counts=[];active=0;contrib=np.zeros(len(SYMS));selection_counts={s:{'long':0,'short':0} for s in SYMS}
 for dt in grid:
  t=ms(dt);nxt=t+7*DAY;cand=[]
  for i,s in enumerate(SYMS):
   d=data[s];r0=d.get(t);r1=d.get(nxt);c1=d.get(t-DAY);c8=d.get(t-8*DAY)
   if r0 is None or r1 is None or c1 is None or c8 is None:continue
   q=[];ok=True
   for k in range(1,31):
    row=d.get(t-k*DAY)
   ...
   if not ok:continue
   liq=float(np.mean(q))
   if liq<10_000_000:continue
   sig=c1[1]/c8[1]-1.;ret=r1[0]/r0[0]-1.
   if math.isfinite(sig) and math.isfinite(ret):cand.append((i,s,sig,liq,ret))
  cand.sort(key=lambda x:(-x[3],x[1]));cand=cand[:40];elig_counts.append(len(cand));desired=np.zeros(len(SYMS))
  if len(cand)>=LOCK['universe']['minimumEligiblePerRebalance']:
   active+=1;ranked=sorted(cand,key=lambda x:(-x[2],x[1]));qn=len(ranked)//5
   longs=ranked[:qn];shorts=ranked[-qn:]
   for i,s,_,_,_ in longs:desired[i]+=.5/qn;selection_counts[s]['long']+=1
   for i,s,_,_,_ in shorts:desired[i]-=.5/qn;selection_counts[s]['short']+=1
  retmap={i:r for i,_,_,_,r in cand};g=0.;by=np.zeros(len(SYMS))
  for i,w in enumerate(desired):
   if w and i in retmap:z=w*retmap[i];g+=z;by[i]=z
 ...
 gs=stats(gross,dates,26100);ns=stats(net,dates,26101);s15=stats(n15,dates,26111);s2=stats(n2,dates,26112);pos=contrib[contrib>0];dom=float(pos.max()/pos.sum()) if len(pos) and pos.sum()>0 else 0.;cov=active/max(1,len(grid));avg=float(np.mean(elig_counts)) if elig_counts else 0.;half=len(net)//2;fh=float(net[:half].sum());sh=float(net[half:].sum())
 g=LOCK['developmentGate'];gate={'minimumEvaluatedWeeks':len(net)>=g['minimumEvaluatedWeeks'],'minimumCoverageFraction':cov>=g['minimumCoverageFraction'],'averageEligibleSymbolsMin':avg>=g['averageEligibleSymbolsMin'],'netMeanWeeklyReturnMin':ns['meanWeeklyReturn']>=g['netMeanWeeklyReturnMin'],'netAnnualizedSharpeMin':ns['annualizedSharpe']>=g['netAnnualizedSharpeMin'],'netProfitFactorMin':ns['profitFactor']>=g['netProfitFactorMin'],'stress1_5xMeanWeeklyReturnMin':s15['meanWeeklyReturn']>=g['stress1_5xMeanWeeklyReturnMin'],'stress2xMeanWeeklyReturnMin':s2['meanWeeklyReturn']>=g['stress2xMeanWeeklyReturnMin'],'positiveMonthFractionMin':ns['positiveMonthFraction']>=g['positiveMonthFractionMin'],'firstHalfNetReturnPositive':fh>0,'secondHalfNetReturnPositive':sh>0,'bootstrapProbabilityPositiveMin':ns['bootstrap']['probPositive']>=g['bootstrapProbabilityPositiveMin'],'bootstrapCi95LowMin':ns['bootstrap']['ci95Low']>=g['bootstrapCi95LowMin'],'maxDrawdownMax':ns['maxDrawdown']<=g['maxDrawdownMax'],'singleSymbolPositiveContributionMax':dom<=g['singleSymbolPositiveContributionMax']};gate={k:bool(v) for k,v in gate.items()};failed=[k for k,v in gate.items() if not v]
 close={'schema':'mgpt_arp26_mom7_development_closeout_v1','generation':'ARP26-MOM7','generatedAtUtc':datetime.now(timezone.utc).isoformat(),'status':'PASS_DEVELOPMENT_GATE' if not failed else 'REJECT_DEVELOPMENT_GATE','productionAuthority':False,'r15MutationAllowed':False,'validationAuthorized':not failed,'sealedHoldoutOpened':False,'lockSha256':hashlib.sha256(LOCK_PATH.read_bytes()).hexdigest(),'development':{'expectedWeeks':len(grid),'evaluatedWeeks':len(net),'activeCoverageWeeks':active,'coverageFraction':cov,'averageEligibleSymbols':avg,'meanTurnover':float(tos.mean()),'singleSymbolPositiveContribution':dom,'firstHalfNetReturn':fh,'secondHalfNetReturn':sh,'gross':gs,'baseCost':ns,'stress1_5x':s15,'stress2x':s2,'symbolGrossContributions':{s:float(contrib[i]) for i,s in enumerate(SYMS)},'selectionCounts':selection_counts},'gate':gate,'failedChecks':failed,'nextAction':'Open unchanged fresh validation only.' if not failed else 'Close ARP26-MOM7; validation and holdout remain sealed.'}
 trans={'schema':'mgpt_arp26_mom7_transport_diagnostics_v1','generation':'ARP26-MOM7','generatedAtUtc':datetime.now(timezone.utc).isoformat(),'checksumRequired':True,'manifestCount':len(manifest),'symbols':diag}
 cp=OUT/'ARP26_MOM7_DEVELOPMENT_CLOSEOUT_20260915.json';tp=OUT/'ARP26_MOM7_TRANSPORT_DIAGNOSTICS_20260915.json';cp.write_text(json.dumps(close,indent=2)+'\n');tp.write_text(json.dumps(trans,indent=2)+'\n');sums=[]
 for p in sorted(OUT.glob('ARP26_MOM7_*_20260915.json')):sums.append(f'{hashlib.sha256(p.read_bytes()).hexdigest()}  {p.name}')
 (OUT/'ARP26_MOM7_SHA256SUMS_20260915.txt').write_text('\n'.join(sums)+'\n');print(json.dumps({'status':close['status'],'failedChecks':failed,'development':{k:v for k,v in close['development'].items() if k not in ('symbolGrossContributions','selectionCounts')}},indent=2))
