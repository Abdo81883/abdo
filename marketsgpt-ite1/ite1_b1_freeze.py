...
LOCK=json.loads((ROOT/"MGPT_ITE1_B1_RANGE_REVERSION_LOCK_20260916.json").read_text())
DATA=ROOT/"data"/"b1_fresh"; DATA.mkdir(parents=True,exist_ok=True)
...
def main():
    ...
    status={"schema":"mgpt_ite1_b1_data_freeze_v1","generatedAtUtc":datetime.now(timezone.utc).isoformat(),
            ...
            "lockSha256":hashlib.sha256((ROOT/"MGPT_ITE1_B1_RANGE_REVERSION_LOCK_20260916.json").read_bytes()).hexdigest(),
            "provider":"Yahoo Finance via yfinance","files":manifest,"errors":errors}
    p=OUT/"MGPT_ITE1_B1_DATA_FREEZE_STATUS_20260916.json";p.write_text(json.dumps(status,indent=2,sort_keys=True)+"\n")
    sums=[f"{r['sha256']}  data/b1_fresh/{r['file']}" for r in manifest]
    sums.append(f"{hashlib.sha256(p.read_bytes()).hexdigest()}  results/{p.name}")
    (OUT/"MGPT_ITE1_B1_DATA_SHA256SUMS_20260916.txt").write_text("\n".join(sums)+"\n")
