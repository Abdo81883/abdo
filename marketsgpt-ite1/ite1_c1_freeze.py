...
LOCK=json.loads((ROOT/"MGPT_ITE1_C1_BREAKOUT_PAYOFF_LOCK_20260916.json").read_text())
DATA=ROOT/"data"/"c1_fresh"; DATA.mkdir(parents=True,exist_ok=True)
...
def min_rows(fam):
    return {"CRYPTO_SPOT":5000,"FX":3500,"COMMODITY_FUTURES_PROXY":2500,"EQUITY":900,"INDEX":900}[fam]
...
def main():
    ...
    for item in LOCK["freshUniverse"]:
        try:
            x=fetch(item["symbol"])
            m=min_rows(item["marketFamily"])
            if len(x)<m:raise RuntimeError(f"rows {len(x)} < required {m}")
        ...
        except Exception as e:errors.append({"symbol":item["symbol"],"marketFamily":item["marketFamily"],"error":str(e)})
    status={"schema":"mgpt_ite1_c1_data_freeze_v1","generatedAtUtc":datetime.now(timezone.utc).isoformat(),
            "status":"PASS_DATA_FREEZE" if not errors else "BLOCKED_DATA_FREEZE","performanceOutcomesComputed":False,
            "validationOpened":False,"holdoutOpened":False,
            "lockSha256":hashlib.sha256((ROOT/"MGPT_ITE1_C1_BREAKOUT_PAYOFF_LOCK_20260916.json").read_bytes()).hexdigest(),
            "provider":"Yahoo Finance via yfinance","files":manifest,"errors":errors}
    p=OUT/"MGPT_ITE1_C1_DATA_FREEZE_STATUS_20260916.json";p.write_text(json.dumps(status,indent=2,sort_keys=True)+"\n")
    sums=[f"{r['sha256']}  data/c1_fresh/{r['file']}" for r in manifest]
    sums.append(f"{hashlib.sha256(p.read_bytes()).hexdigest()}  results/{p.name}")
    (OUT/"MGPT_ITE1_C1_DATA_SHA256SUMS_20260916.txt").write_text("\n".join(sums)+"\n")
